import {registerBlockType} from '@wordpress/blocks';
import domReady from '@wordpress/dom-ready';

domReady(function () {

    let __ = window.wp.i18n.__;

    registerBlockType(
        'travels/existing-product',
        {
            title: __('Existing Product', 'travels'),
            icon: 'dashicons-align-wide',
            category: 'layout',
            example: {},
            edit: function ( props ) {
                return (
                    <section id="product-feedback"
                             className="section novi-bg novi-bg-img section-xl text-center bg-accent">
                        <div className="container">
                            <div className="row row-fix justify-content-lg-center">
                                <div className="col-md-8 text-center">
                                    <h4>Feedback on existing itinerary</h4>
                                </div>
                            </div>
                            <div className="row row-30">
                                <div className="col-md-8 text-left">
                                    <p>
                                        You may already have booked and planned your travel to Greenland either with a
                                        travel
                                        agency
                                        or by your selves.
                                    </p>
                                    <p>
                                        In both cases we are more than happy to go through the itinerary to see if we
                                        believe
                                        something has been missed or to tip you with additional tours or experiences,
                                        that we
                                        think you cannot be allowed to miss, when travelling in Greenland.
                                    </p>
                                    <h5>
                                        How it works:
                                    </h5>
                                    <ul className="list-marked">
                                        <li className="text-white">You forward us the entire itinerary and we look
                                            through and return with our feedback
                                            within 72 hours.
                                        </li>
                                        <li className="text-white">You are allowed two follow-up e-mails with questions
                                            about your itinerary
                                            feedback.
                                        </li>
                                    </ul>
                                </div>
                                <div className="col-md-4">
                                    <div className="my-auto">
                                        <div className="pricing-table">
                                            <div className="pricing-label"><span></span>
                                                <svg width="86px" height="86px" viewBox="0 0 86 86">
                                                    <path
                                                        d="M73.4,73.4L67.3,73l-0.8,6.2l-6-1.8l-2.1,5.9l-5.5-3l-3.2,5.3l-4.7-4L40.7,86L37,81.1l-5.1,3.5L29.2,79            l-5.7,2.4l-1.4-6l-6.1,1.1l-0.2-6.2l-6.2-0.2l1.1-6.1l-6-1.4L7,56.8l-5.6-2.7L4.9,49L0,45.3L4.5,41l-4-4.7L5.8,33l-3-5.5l5.9-2.1            l-1.8-6l6.2-0.8l-0.5-6.2l6.2,0.5l0.8-6.2l6,1.8l2.1-5.9l5.5,3l3.2-5.3l4.7,4L45.3,0L49,4.9l5.1-3.5L56.8,7l5.7-2.4l1.4,6l6.1-1.1            l0.2,6.2l6.2,0.2L75.3,22l6,1.4L79,29.2l5.6,2.7L81.1,37l4.9,3.8L81.5,45l4,4.7L80.2,53l3,5.5l-5.9,2.1l1.8,6L73,67.3L73.4,73.4z"></path>
                                                </svg>
                                            </div>
                                            <div className="pricing-header">
                                                <h5>Feedback on existing itinerary</h5>
                                                <div className="price"><span
                                                    className="text-white">$80</span><span></span></div>
                                            </div>
                                            <div className="pricing-footer">
                                                <a className="button button-primary" href="#contact-us">
                                                    Contact us
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </section>
                );
            },
            save: function ( props ) {
                return (
                    <section id="product-feedback"
                             className="section novi-bg novi-bg-img section-xl text-center bg-accent">
                        <div className="container">
                            <div className="row row-fix justify-content-lg-center">
                                <div className="col-md-8 text-center">
                                    <h4>Feedback on existing itinerary</h4>
                                </div>
                            </div>
                            <div className="row row-30">
                                <div className="col-md-8 text-left">
                                    <p>
                                        You may already have booked and planned your travel to Greenland either with a
                                        travel
                                        agency
                                        or by your selves.
                                    </p>
                                    <p>
                                        In both cases we are more than happy to go through the itinerary to see if we
                                        believe
                                        something has been missed or to tip you with additional tours or experiences,
                                        that we
                                        think you cannot be allowed to miss, when travelling in Greenland.
                                    </p>
                                    <h5>
                                        How it works:
                                    </h5>
                                    <ul className="list-marked">
                                        <li className="text-white">You forward us the entire itinerary and we look
                                            through and return with our feedback
                                            within 72 hours.
                                        </li>
                                        <li className="text-white">You are allowed two follow-up e-mails with questions
                                            about your itinerary
                                            feedback.
                                        </li>
                                    </ul>
                                </div>
                                <div className="col-md-4">
                                    <div className="my-auto">
                                        <div className="pricing-table">
                                            <div className="pricing-label"><span></span>
                                                <svg width="86px" height="86px" viewBox="0 0 86 86">
                                                    <path
                                                        d="M73.4,73.4L67.3,73l-0.8,6.2l-6-1.8l-2.1,5.9l-5.5-3l-3.2,5.3l-4.7-4L40.7,86L37,81.1l-5.1,3.5L29.2,79            l-5.7,2.4l-1.4-6l-6.1,1.1l-0.2-6.2l-6.2-0.2l1.1-6.1l-6-1.4L7,56.8l-5.6-2.7L4.9,49L0,45.3L4.5,41l-4-4.7L5.8,33l-3-5.5l5.9-2.1            l-1.8-6l6.2-0.8l-0.5-6.2l6.2,0.5l0.8-6.2l6,1.8l2.1-5.9l5.5,3l3.2-5.3l4.7,4L45.3,0L49,4.9l5.1-3.5L56.8,7l5.7-2.4l1.4,6l6.1-1.1            l0.2,6.2l6.2,0.2L75.3,22l6,1.4L79,29.2l5.6,2.7L81.1,37l4.9,3.8L81.5,45l4,4.7L80.2,53l3,5.5l-5.9,2.1l1.8,6L73,67.3L73.4,73.4z"></path>
                                                </svg>
                                            </div>
                                            <div className="pricing-header">
                                                <h5>Feedback on existing itinerary</h5>
                                                <div className="price"><span
                                                    className="text-white">$80</span><span></span></div>
                                            </div>
                                            <div className="pricing-footer">
                                                <a className="button button-primary" href="#contact-us">
                                                    Contact us
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </section>
                );
            }
        }
    )
})